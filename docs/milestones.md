...
## Poison Piranha Plant capture removal (issue #281)

2026-08-17: dropped the `Poison Piranha Plant` capture item from [items.json](../apworld/smo_archipelago/data/items.json) and the matching `Capture: Poison Piranha Plant` location from [locations.json](../apworld/smo_archipelago/data/locations.json). Pool goes 67 → 66 items, 478 → 477 locations, 42 → 41 captures.

**Why**: capturing a Poison Piranha Plant requires feeding it a rock first — Cappy alone just gets swallowed. The only spot in the game with both plants and throwable rocks is the Wooded Kingdom Iron Road (near the *Iron Road: Halfway Point* flag), which is **first-mission content that despawns once the Wooded story is finished**. Metro's Underground Power Plant has plants but no rocks (and its generator-room pair are Big Poison Piranha Plants, which can't be captured at all). The only re-open is post-game: clear Bowser → activate Wooded's Moon Rock → the moon pipe near the Secret Flower Field Entrance opens the Invisible Road subarea, which does have rocks.

The old rule was `({WoodedPeace()}) AND |Poison Piranha Plant|` — i.e. `|Uproot| and |Sherm|`, which is *exactly* the condition under which the capture stops being obtainable. AP logic is monotonic and can't model a window that closes, so any player who received the item after finishing Wooded was locked out until post-game; under `goal: festival` post-game never happens, making it permanently unobtainable and a hard-lock risk if a progression item landed there. Loosening the rule to Iron Road access (`{WoodedDifficult()}`) would fix the false Sherm gate but not the closing window, so the location was cut instead.

**Switch-mod side needs no change**: `captureBitFor` returns `0xff` for a cap that isn't in the generated table, and both [CaptureGate::captureBlocked](../switch-mod/src/game/CaptureGate.cpp) and [AddHackDictionaryHook](../switch-mod/src/hooks/AddHackDictionaryHook.cpp) fail *open* on `0xff`. So after regenerating `capture_table.h` the plant is simply a free capture — capturable any time, and its Capture List entry still writes through on capture. It is deliberately NOT added to `kBaselineHacks` (those three are pre-populated into the compendium without capturing; the plant should still be earned).

**Follow-ups when this lands**: re-run `scripts/sync_capture_table.py` (bit indices after Puzzle Part shift down by one — internal to a single build, not a wire contract) and rebuild the PopTracker pack, which derives its capture toggles and access rules from the same JSON.
