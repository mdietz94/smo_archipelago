...
## M9 — exlaunch → LibHakkun migration

2026-05-21: **DONE.** Switch-side stack migrated from exlaunch + lunakit-vendor (subsdk9, devkitA64 toolchain, hand-mangled `HookSymbols.hpp`) to LibHakkun + OdysseyHeaders + sail (subsdk9, LLVM 19 + libc++ toolchain, sail `.sym` symbol DB). Landed in two phases:

- **PR #151 (2026-05-21)** — parallel `switch-mod-hk/` tree at subsdk8 alongside the still-shipping `switch-mod/` exlaunch tree. Validated end-to-end on Ryujinx (loopback test, kingdom entry, Cappy speech bubbles) + real-Switch FW22 manual play. 26 production hooks ported as `HkTrampoline` + `installAtSym<>`; one inline-at-offset hook (`CreditsStartHook`) deferred until the post-merge fix.
- **Cutover PR (the one introducing this entry)** — `switch-mod-hk/` → `switch-mod/`, subsdk8 → subsdk9, exlaunch + lunakit-vendor submodules removed. CI workflows + skills + docs swept. FW22 unsupported claims relaxed in CLAUDE.md + docs/first-time-setup.md + wizard.py.

### Net effects

- **M6.1 worker-thread allocator constraint retired.** LibHakkun's musl + LLVM libc++ + `HeapSourceDynamic` addon (which routes `operator new` / `malloc` / `free` to SMO's own thread-safe allocator) clears the libstdc++ TLS-slot NULL-deref class. Worker threads can now use `std::set` / `std::vector` / `std::string` / `std::mutex` freely. The defensive patterns (`FlatHashSet<N>`, `LineBuffer`, `copyFixedFieldN`, snprintf-to-stack-`char[24]`, release-store-publish atomics) stay in tree as vestigial code; phase 7 polish will retire them.
- **Sail .sym DB complements `HookSymbols.hpp`.** New entries land in both `switch-mod/syms/game/SmoApSymbols.sym` (sail link-side; or `switch-mod/syms/nn/<lib>.sym` for system libs) and `switch-mod/src/hooks/HookSymbols.hpp` (the `inline constexpr` name strings used at C++ call sites); `llvm-nm --dynamic build/fakesymbols.so` catches drift at build time. `scripts/check_nso_symbols.py` retired (sail's link-side check replaces it).
- **FW22 confirmed supported.** Real-Switch boot of SMO 1.0.0 + Hakkun subsdk9 on FW22 succeeded during both the 2026-05-20 spike and the post-PR-#151 validation.

### Five real bugs fixed during the port

1. **AArch64 PC-relative trampoline relocator** in upstream `LibHakkun`'s `HkTrampoline` ([sys/hakkun/include/hk/hook/Trampoline.h:67](../switch-mod/sys/hakkun/include/hk/hook/Trampoline.h)). Upstream copies the original first instruction verbatim into the trampoline pool; `adrp` / `adr` / `b` / `bl` / `b.cond` / `cbz` / `tbz` / `ldr-literal` at function entry then execute at the wrong PC and the guest crashes. Worth upstreaming to fruityloops1/LibHakkun — patch in [scripts/patch_hakkun.py](../scripts/patch_hakkun.py) (patches 7a/b/c). See [project_hakkun_trampoline_no_pcrel_reloc](../../.claude/projects/C--Users-maxwe-Documents-smo-archipelago/memory/project_hakkun_trampoline_no_pcrel_reloc.md).
2. **`sm::ServiceManager::initialize()` is non-lazy.** The spike's Gate 2 only took the function's address, never called it; ApClient::initNetworking has to explicitly init sm before bringing up `hk::socket`.
3. **Mangled-symbol length-prefix typo.** Sail emits whatever you write into `fakesymbols.so`; the linker resolves the typo'd name; the runtime null-derefs because the typo isn't in main.nso's dynsym. Hit during `StaffRollScene` (15→14) and nifm (28→27 / 19→18) entries. See [project_sail_mangling_length_trap](../../.claude/projects/C--Users-maxwe-Documents-smo-archipelago/memory/project_sail_mangling_length_trap.md).
4. **`IUseSceneObjHolder` multi-inheritance offset adjustment** — 19 bisect cycles. `al::Scene` multiply-inherits from `NerveExecutor`, `IUseAudioKeeper`, `IUseCamera`, `IUseSceneObjHolder`; `rs::isActiveCapMessage` / `rs::tryShowCapMessagePriorityLow` take `IUseSceneObjHolder*` and vtable-dispatch. Production exlaunch `static_cast<IUseSceneObjHolder*>(scene)`'d so the compiler inserted the offset; the phase-3b port had a TODO comment "doesn't matter" — wrong. Surfaced as Ryujinx ARMeilleure 0xC0000005. Fix: commit `4ff5864`.
5. **CappyMessenger worker→frame race.** Worker calls to `CappyMessenger::enqueueSystem` were writing to non-atomic `queue_[]/tail_/live_count_` from the worker thread while `tryPump` read+wrote them on the frame thread. Routed through a new `inbound_system_bubbles` SPSC ring drained from `drawMain`. Also landed against production switch-mod as PR #147.

### CreditsStartHook post-merge

The PR-#151 body deferred `CreditsStartHook` "until first real-Switch boot validates", but commit `a422098` (post-#151, pre-cutover) brought it back as a Strategy A inline BL patch at `+0x4C54A4`. Hakkun's `writeBranchLinkAtMainOffset` replaces the exlaunch-era `HOOK_DEFINE_INLINE` macro. Goal detection is live again.
