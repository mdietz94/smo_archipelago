...
- Bridge IP/port baked at compile time (cmake `-DBRIDGE_HOST=...`) as a fallback only — runtime UDP discovery (see "Configuring the bridge target" above) is the primary path and handles routine LAN-IP changes automatically. The historical attempt to read `romfs/ap_config.json` at runtime was rolled back when `nn::fs::MountSdCardForDebug` turned out to be broken on retail firmware — fine in Ryujinx, fails on real hardware. Re-running the setup wizard is only required if UDP discovery is blocked AND the baked-in fallback no longer matches your PC.
...
| No `switch HELLO` in SMOClient log | PC firewall blocking UDP 17776 (discovery) and/or TCP 17777, NIFM not up on Switch, or — only if discovery is also failing — a stale baked-in fallback IP after a network move | Add firewall rules (`New-NetFirewallRule -DisplayName "SMO AP TCP" -Direction Inbound -Protocol TCP -LocalPort 17777 -Action Allow` and the matching UDP/17776 rule); check mod log for `nn::nifm::Initialize failed` or `connect failed` lines; only if UDP is genuinely blocked, type `/setup` in SMOClient to rebuild with the current LAN IP |
