...
The forked apworld registers as `Spicy Meatball Overdrive` (vs. the Manual client's `Manual_SMO_mp3`). The client's `Connect` packet uses this exact game name. Seeds generated by either world are not interchangeable; that's intentional — the real-client seeds gain richer enforcement options the Manual client cannot honor. The Python package folder is still `smo_archipelago/` (and therefore the host.yaml settings key is still `smo_archipelago_options`) — renaming the folder would break in-flight Archipelago checkouts that already have the apworld unpacked, so the on-disk identifier and the user-facing game name intentionally differ.
