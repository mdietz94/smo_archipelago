...
| `meatballs/_setup/switch_mod/...` | `switch-mod/` + `switch-mod/sys` (LibHakkun) + `switch-mod/lib/OdysseyHeaders` submodules | C++ source tree the wizard compiles on the user's machine |
| `meatballs/_setup/scripts/...` | `scripts/extract_shine_map.py` + `sync_capture_table.py` + `build_switchmod.py` + `patch_hakkun.py` + `setup_sail_winpath.py` + `fix_hakkun_symlinks.py` | Extractor + build helpers the wizard invokes |

Size budget is about 25 MB compressed (most of it LibHakkun + OdysseyHeaders sources).
...
- The wizard actually runs end-to-end on a clean
  ...
  unit-tested — needs real LLVM 19 + msys2 mingw64 + hactool + an NSP)
