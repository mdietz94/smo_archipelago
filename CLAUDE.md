...
| **Bit-index capture table generated from apworld** | `scripts/sync_capture_table.py` regenerates `switch-mod/src/ap/capture_table.h` from `data/items.json` so Switch and bridge can't drift on cap-name → bit-index assignment. Companion `scripts/sync_shine_table.py` regenerates `switch-mod/src/ap/shine_table.h` from `data/locations.json` × `client/data/shine_map.json`; both generated headers are gitignored (Nintendo-IP cross-references — see the IP section above) and are regenerated by the wizard (`run_sync_capture_table` + `run_sync_shine_table` in `_setup/build.py`) before every switch-mod build |
